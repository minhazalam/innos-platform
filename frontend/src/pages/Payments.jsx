...
export default function Payments() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Payments" subtitle="All recorded transactions" />

      <Card className="flex items-center justify-between rounded-2xl border-primary/20 bg-primary/5 p-5">
        <div>
          ...
          {data.map((p) => (
            <Card key={p.id} data-testid={`payment-row-${p.id}`} className="flex items-center justify-between rounded-xl border-border p-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><ArrowDownLeft className="h-5 w-5" /></div>
                <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-display text-lg font-bold">{inr(p.amount)}</span>
              </div>
            </Card>
          ))}
        </div>
